  ...
  return (
    <div style={{ marginTop: '100px', display: 'flex', justifyContent: 'center' }}>
      <Paper sx={{ padding: '20px 50px'}}>
        ...
        {props.products && (
          <p>Total: ${props.products.reduce((sum, entry) => { return sum + (entry.price * entry.amount) }, 0).toFixed(2)}</p>
        )} 
        <Box sx={{display: 'flex', flexDirection: 'column', alignItems: 'center' }} component="form" onSubmit={handleSubmit} autoComplete="off">
          <Box sx={{display: 'flex', flexDirection: 'row'}} mt={1}>
            <Box mr={2} sx={{display: 'flex', flexDirection: 'column'}}>
              <Typography mt={3} variant="h6">Billing Address</Typography>
              <TextField
                mt={2}
                id="full-name"
                variant="standard"
            ...
            <Box ml={2} sx={{display: 'flex', flexDirection: 'column'}}>
              <Typography mt={3} variant="h6">Payment Details</Typography>
              <TextField
                mt={2}
          ...
          <Button sx={{ marginTop: '20px', width: '300px' }} type="submit" variant="contained">Place Your Order</Button>
        </Box>
      </Paper>
    </div>
  );
...
const handleSubmit = (e) => {
  ...
  swal('Order Complete', `Thank you ${e.target[0].value} for your patronage!`, 'success');
}
